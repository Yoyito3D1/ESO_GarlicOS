...
_gs_borrarVentana:
	...
	and r0, #0x0F				@; filtra número de ventana (0..15)
	ands r1, #1					@; filtra parámetro R1 (0 o 1)
	andeq r0, #0x03				@; segundo filtro de número de ventana
								@; (0..3, si R1==0)
	mov r2, #24
	mov r2, r2, lsl #7
...
.LiniGrafB_for1:
 ...
 @; Generar baldosas de colores
	...
	mov r2, #0x2000				@; R2 = 128 * 64 (núm. bytes baldosas)
...
.LpintarFranjas_for:			@; bucle para 4 píxeles verticales por franja
	...
	tst r4, #1
	andne r7, #0xFF				@; paridad índice columna impar:
	orrne r7, r1					@; limpiar byte alto, fijar byte alto
	biceq r7, #0xFF				@; paridad índice columna par:
	...
	add r4, #1					@; avanzar índice de columna
	cmp r4, #8					@; verificar si se llega a la última columna
	moveq r4, #0
...
.LpintarFranjas_fiwhile:
	...
	@; Rutina para para representar gráficamente la ocupación de las pilas
	@; de los procesos de usuario, además de la pila del proceso de control del
...
.LrP_SPn0:
	...
	mov r2, r2, lsr #3		@; R2 = número de marcas de pila (hasta 16)
	b .LrP_cont0			@; continua los cálculos como un proceso normal

	@; bucle para cálculo y visualización de la ocupación de la pila
.LrP_for0:
	ldr r5, [r4]			@; R5 = PID del proceso del zócalo actual
	cmp r5, #0
	beq .LrP_cont3			@; evitar cálculos si PID = 0 (excepto para S.O.)
